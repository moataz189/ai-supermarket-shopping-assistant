...
alertmanager:
  config:
    ...
    inhibit_rules:
      ...
      - target_matchers:
          - 'alertname = InfoInhibitor'
      # New: BackendHighErrorRate/BackendCriticalErrorRate (and MCPServiceHighFailureRate/
      # MCPServiceUnavailable) are deliberately different alertnames, so none of the four
      # rules above (all keyed on matching `alertname`) apply between them. Matching on
      # `service` instead covers any warning/critical pair sharing a `service` label, not
      # hardcoded to one service specifically.
      - source_matchers:
          - 'severity = "critical"'
        target_matchers:
          - 'severity = "warning"'
