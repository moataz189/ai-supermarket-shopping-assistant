#!/bin/bash
# Launch-template user-data for kubeadm worker nodes (every worker ASG instance runs
# this). Adapted from polyaifursa's infra/tf/modules/k8s-cluster/scripts/worker.sh: same
# CRI-O start + SSM-polling join pattern (no manual `kubeadm join` copy/paste, no
# dependency on Terraform apply ordering — this loop is what actually waits for the
# control plane). CRI-O/kubelet/kubeadm/kubectl assumed already installed by
# infra/packer/install-k8s-deps.sh.
set -euo pipefail
